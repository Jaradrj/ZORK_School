...
public class TeacherRoom implements Room {
    ...
    public void enter(Player player) {
        ...
        if (lastRoom.equals("Main Entrance Hall")) {
            lightIsOn = true;
            hasPerson = true;
        }

        if ((lastRoom.equals("IT Room") || lastRoom.equals("Music Room")) && !player.hasFlag("has_followed_teacher")) {
            hasPerson = false;
            player.setFlag("saw_teacher_leave");
        }
        System.out.println(lightIsOn ? "The lights are on, illuminating the room." : "The room is dark.");

        if (hasPerson) {
            System.out.println("A woman sits at the desk, sipping something from a steaming mug. She looks alot like Mrs. Hamps, your school psychologist.\nPoor woman, must have been working really hard helping all the missing students friends and family.");
            System.out.println("Actions:\n1. Talk to her\n2. Leave");
        } else if (player.hasFlag("saw_teacher_leave") && !player.hasFlag("has_followed_teacher")) {
            ...
            System.out.println("The room is empty. A hot cup sits on the table. A laptop screen glows faintly. There's Papers all over the Head Teacher's desk.\n Some were also tossed in the trash bin. The science award diploma is proudly presented on top of the desk. Next to it, you see a Flashlight.");
            System.out.println("Actions:");
